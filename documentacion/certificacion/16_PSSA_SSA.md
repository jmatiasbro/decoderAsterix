...
| **STCA-3** — parrots no excluidos + falso negativo en tráfico lento | **CERRADO** (2026-07-09) | El chequeo `mode3a == '0000'` era inefectivo (squawk 0000 normalizado a `''`): los parrots generaban *nuisance* STCA, y la única protección real (filtro de velocidad 40 kt) causaba **falso negativo** en tráfico lento real (helicóptero en estacionario). Cerrado por exclusión de parrots **por identidad** (`is_parrot`, LLR-STC-10 / HLR-STCA-09), independiente de la velocidad → permite `velocidad_min_kt=0` protegiendo al tráfico lento. Verificado: `test_stca_scenarios.py` (parrots por identidad, tráfico lento protegido) |
| **STCA-4** — nuisance vertical en RVSM | **CERRADO** (2026-07-09) | Umbral vertical de Ruta bajado de 1000 → **800 ft** (LLR-STC-04 / HLR-STCA-02): con 1000 ft el tráfico correctamente separado en RVSM disparaba STCA por la cuantización Mode C (falsas alarmas sistemáticas que erosionan la confianza en la red). 800 ft alerta solo ante pérdida real; fallback seguro se mantiene en 1000 ft |
...
| 0.5 | 2026-07-09 | Nuevos hallazgos **STCA-3** (parrots no excluidos + falso negativo en tráfico lento) y **STCA-4** (nuisance vertical RVSM), ambos **CERRADOS** (exclusión por identidad `is_parrot` y vertical de Ruta 800 ft). Refuerzan FC-STCA-01 (falso negativo) y la disponibilidad de la red (nuisance que erosiona la confianza). |
