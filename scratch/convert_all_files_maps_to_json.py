...
def parse_map_recursive(filepath: str, parsed_files: set) -> List[Dict[str, Any]]:
    ...
    while idx < len(lines):
        ...
        # FixPointSymbol, FixPointSymbo, FixpointRouSimbolo
        elif line_lower.startswith('fixpointsymbol') or line_lower.startswith('fixpointsymbo') or line_lower.startswith('fixpointrousimbolo'):
            parts = line.split()
            if len(parts) >= 2:
                coord_str = parts[1].strip()
                label_str = ""
                # Extract label if available (some have it at index 3, e.g. FixPointSymbol coord 212 AKNOS)
                if len(parts) >= 4:
...
def main():
    ...
    print(f"Scanning files directory recursively for JSON conversion: {files_dir}")
    ...
    for root, dirs, files in os.walk(files_dir):
        for file in files:
            if file.lower().endswith(".map"):
                ...
                json_path = map_path.replace(".map", ".json")
                ...
                if features:
                    ...
                    print(f"Converted: {file} -> {os.path.basename(json_path)} ({len(features)} features)")
                    count_converted += 1
                else:
                    print(f"Skipped/Empty: {file}")
                    
    print(f"\n--- JSON TRANSFORMATION COMPLETE ---")
    print(f"Successfully transformed {count_converted} airspace charts to premium JSON format!")
