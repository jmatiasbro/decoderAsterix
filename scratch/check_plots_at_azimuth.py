...
def search_plots_at_azimuth(filepath, target_az=215.6, tolerance=0.5):
    print(f"=== Searching for Plots near Azimuth {target_az}° (Tol +/- {tolerance}°) in: {os.path.basename(filepath)} ===")
            ...
            while offset < len(data):
                ...
                if cat == 48:
                    ...
                    while curr_off < end_offset:
                        ...
                        if theta is not None and abs(theta - target_az) <= tolerance:
                            matches.append({
                                'pcap_ts': ts,
                                'tod': tod,
                                'squawk': squawk,
...
if __name__ == "__main__":
    search_plots_at_azimuth('c:/documentos/decode_asterix/MTR_2026_04_16_16-28-16.pcap', 215.6, 0.5)
